...
class GPCurvesReader:
    ...
    def _gaussian_kernel(self, X: torch.tensor, length: torch.tensor, sigma: float, sigma_noise: float=1e-2) -> torch.tensor:
        ...
        X1 = copy.deepcopy(X)[:, None, :, :] # (batch_size, 1, num_total_points, x_size)
        X2 = copy.deepcopy(X)[:, :, None, :] # (batch_size, num_total_points, 1, x_size)
        diff = X1 - X2 # (batch_size, num_total_points, num_total_points, x_size)
    ...
    def generate_curves(self): 
        """Generate functions are `float32` with x values between -2 and 2.
        ...
        num_context = random.randint(3, self.max_num_context) # randomly select from U[3, max_num_context]
        ...
        if self._testing: 
            ...
            x_values = torch.tile(
                torch.unsqueeze(torch.arange(-2., 2., 1./100, dtype=torch.float32), dim=0), 
                [self._batch_size, 1])
        ...
        # When training, the number and position of context and target points are randomly selected. 
            ...
            num_target = random.randint(2, self.max_num_context) # randomly select from U[2, max_num_context]
            num_total_points = num_context + num_target
            
            x_values = 4 * torch.rand(self._batch_size, num_total_points, self._x_size) - 2 # between -2 and 2
        ...
        cholesky = torch.linalg.cholesky(kernel.double()).float() # (batch_size, y_size, num_total_points, num_total_points)
        ...
        y_values = torch.transpose(torch.squeeze(y_values, -1), 1, 2) # (batch_size, num_total_points, y_size)
